 ...
 *
 * Restricciones:
 * - imageUri: debe ser una URI válida (file://)
 * - mimeType: solo JPEG o PNG
 * - maxSizeKb: máximo 5MB (5000KB)
 */
            ...
            .refine((val) => val === 'image/jpeg' || val === 'image/png', {
                message: 'Solo se permiten JPEG o PNG',
            }),

            maxSizeKb: z.number().max(5000).optional(),
            }).strict();
 ...
 */
            ...
            async execute(input: UploadAvatarInput): Promise<string> {
                ...
                if (validatedInput.maxSizeKb && validatedInput.maxSizeKb > 5000) {
                    throw new AppError('VALIDATION_ERROR', 'La imagen excede el tamaño máximo permitido (5MB)', 400);
                }
